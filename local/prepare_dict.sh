...
# Modified by Hyunsoo Son 2021
# Prepare dictionary

if [ $# -ne 2 ]; then
  echo "Usage: $0 <lm-dir> <dst-dir>"
  echo "e.g.: /data/local/lm data/local/dict"
  exit 1
fi
lm_dir=$1
dst_dir=$2
...
lexicon_raw_nosil=$lm_dir/lexicon_nosil.txt
...
awk '{for (i=2; i<=NF; ++i) { print $i }}' $lexicon_raw_nosil |\
  sort -u |\
  perl -e 'while(<>){
...
#  echo '<UNK> NSN' ) | \
(echo '!SIL SIL'; echo '<SPOKEN_NOISE> SPN'; echo '<UNK> SPN'; echo '▁  SIL') |\
  cat - $lexicon_raw_nosil | sort > $dst_dir/lexicon.txt
